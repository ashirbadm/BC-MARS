...
#include "myutils2.h"
//#include "worklistc.h"
double cpu_ratiotime = 0.0,gpu_ratiotime = 0.0;
...
void *cpu_BFS(void *P){
	...
	//printf("My border count : %d\n",borderCount);
	/* Do CPU BFS calculate border distance vector*/			
        //printf("initnodesigmadist_omp,%d\n",omp_get_num_threads());
	 initnodesigmadist_omp (source,graph->nnodes, srcpart->nodesigma, srcpart->nodedist,num_threads);
	  starttime = rtclock();
//	printf("cpu_component source = %d, threads : %d\n",source,omp_get_num_threads());
	   cpu_component (srcpart->psrc,srcpart->noutgoing,srcpart->edgesrc,srcpart->edgedst,graph->nnodes,numEdges_src,srcpart->nodedist,srcpart->nodesigma,srcpart->edgesigma,1,&source,var->lock,false,num_threads,*graph);
//	printf("cpu_component over\n");
	endtime = rtclock ();
...
cudaMalloc((void **)&edgesrc, (1000) * sizeof(unsigned)); 
int num_threads=16;
 Graph graph;
 cudaDeviceReset();
...
//      kconf.setProblemSize(graph.maxEdgeCount);
	...
	for(int lock_i=0;lock_i<graph.nnodes;lock_i++)
                omp_init_lock((writelock+lock_i));
...
printf("ii= %d\n",ii);
	...
	if(gpu_ratiotime/(cpu_ratiotime+gpu_ratiotime) > 0.05){
