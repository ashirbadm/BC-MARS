...
*/
Worklist wl1(2), wl2(2), wl_visited(2);
...
__device__
foru processedge2(foru *dist, unsigned hnodes,unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc,unsigned *edgesdst,unsigned iteration, unsigned edge, unsigned &dst, unsigned *border, unsigned *child_count, int *child) {
...
*/
//		if(dst == 845200)
//			printf("%d(%d,%d)->%d(%d,%d) detected\n",src,dist[src],nodesigma[src],dst,dist[dst],nodesigma[dst]);
	//	if(atomicCAS(edgesigma+edge,0,1)==0 && dist_dst >= iteration)
		if(dist_dst >= iteration)
		{
			if(border[dst]!=0)
			{
				if(border[dst] > border[hnodes])	//No equality : border[dst]-1 is true borderIndex
					child[ tex1Dfetch(row_offsets, src) + atomicAdd(child_count+src,1) ] = dst;
				return 0;
			}
			dist[dst] = iteration;
			atomicAdd(nodesigma+dst,sigma_src);
...
//			child[num_edges_v+(child_count[v]++)] = w;
		//	if(dst == 845200 || dst == 845201 || dst == 845202)
//				printf("[GPU %d]src = %d(%d,%d), sigma[%d] = %d, dist[%d] = %d, dist_dst = %d\n",iteration,src,dist[src],nodesigma[src],dst,nodesigma[dst],dst,dist[dst],dist_dst);
                        //if(border[dst]==0)
	//		if(dist_dst > iteration)
				return 1;
		}
...
__device__
foru processedge2_SS(foru *dist, unsigned hnodes,unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc,unsigned *edgesdst,unsigned iteration, unsigned edge, unsigned &dst, unsigned *border) {
...
dist_dst = dist[dst];
	...
	//	if(atomicCAS(edgesigma+edge,0,1)==0 && dist_dst >= iteration)
      			if(dist_dst >= iteration)
                {
                        ...
                        atomicAdd(nodesigma+dst,sigma_src);
                //      if(dst == 845200 || dst == 845201 || dst == 845202)
	                //printf("[GPU %d]src = %d(%d,%d), sigma[%d] = %d, dist[%d] = %d\n",iteration,src,dist[src],nodesigma[src],dst,nodesigma[dst],dst,dist[dst]);
                        if(border[dst]==0)
                        return 1;
                }
            //}
...
__device__
unsigned processnode2(foru *dist, unsigned hnodes,unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc,unsigned *edgesdst,unsigned *noutgoing , Worklist &inwl, unsigned iteration, unsigned *border, unsigned *child_count, int *child) 
{
...
;
  ...
  int total_inputs = (inwl.countVerticesOnLevel(iteration-1) + gridDim.x * blockDim.x - 1)/(gridDim.x * blockDim.x);
  
  while(total_inputs > 0)
    {      
	total_inputs -= (gridDim.x * blockDim.x);
      int neighborsize = 0;
      int neighboroffset = 0;
      ...
      if(inwl.pop_id(id, nn, iteration-1) )
	{	  
	  if(nn != -1 && atomicCAS(edgesigma+nn,0,1)==0)
	    {
      ...
      while(total_edges > 0)
	{
	  ...
	  if(threadIdx.x < total_edges)
	    {
	      if(processedge2(dist, hnodes, nodesigma,edgesigma,srcsrc,edgesdst,iteration, gather_offsets[threadIdx.x], to_push, border, child_count, child))
	...
	inwl.push_1item<BlockScan>(ncnt, (int) to_push, BLKSIZE);
...
__device__
unsigned processnode2_SS(foru *dist, unsigned hnodes,unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc,unsigned *edgesdst,unsigned *noutgoing , Worklist &inwl, Worklist &outwl, unsigned iteration, unsigned *border)
{
  //expandByCTA(dist, graph, inwl, outwl, iteration);
      ...
      while(total_edges > 0)
        {
          ...
          if(threadIdx.x < total_edges)
            {
              if(processedge2_SS(dist, hnodes, nodesigma,edgesigma,srcsrc,edgesdst,iteration, gather_offsets[threadIdx.x], to_push, border))
              //if(processedge2(dist, hnodes, iteration, gather_offsets[threadIdx.x], to_push))
                {
                  ncnt = 1;
                }
        ...
        //  printf("%u ", to_push);
          ...
          outwl.push_1item<BlockScan>(ncnt, (int) to_push, BLKSIZE);
        //wl_visited.push_1item<BlockScan>(ncnt, (int) to_push, BLKSIZE);
	total_edges -= BLKSIZE;
          done += BLKSIZE;
        }
...
*/
__device__
void drelax(foru *dist, unsigned hnodes, unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc, unsigned *edgesdst,unsigned *noutgoing,unsigned *gerrno, Worklist &inwl, int iteration, unsigned *border, unsigned *child_count, int *child) {
	//unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
/*
	if(iteration == 0)
	  {
	  ...
	  {
	 */
	    if(processnode2(dist, hnodes,nodesigma,edgesigma,srcsrc, edgesdst,noutgoing,inwl, iteration, border, child_count, child))
	      *gerrno = 1;
	  //}
}

__device__
void drelax_SS(foru *dist, unsigned hnodes, unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc, unsigned *edgesdst,unsigned *noutgoing,unsigned *gerrno, Worklist &inwl, Worklist& outwl, int iteration, unsigned *border) {
       
            if(processnode2_SS(dist, hnodes,nodesigma,edgesigma,srcsrc, edgesdst,noutgoing,inwl, outwl, iteration, border))
              *gerrno = 1;
...
__device__ void clock_block( clock_t clock_count, clock_t* d_o)
{
printf("clock_block start\n");
    ...
    while (clock_offset < clock_count)
    {
     ...
     d_o[0] = clock_offset;
	printf("clock_block over\n");
}


__device__ int addSourcesOnCurrentLevel(foru *dist, int *d_sources, unsigned processed_count, Worklist &inwl, unsigned &iteration, int *d_source_sigma, unsigned *nodesigma, int *flag)
{
        int id = processed_count, item, dist_item;
        item = d_sources[id];
//	printf("d_sources[%d] = %d in iteration %d\n", id, d_sources[id],iteration);
	if(item == -99999)
	{
		atomicCAS(flag,0,1);
		atomicCAS(&d_sources[id],-99999,-iteration);		
		printf("[GPU] Set d_sources[%d] as %d\n",id,d_sources[id]);
	}
	else if(item == -100000)	return id;
	else if(item == -iteration)
	{
//		printf("elseif iteration = %d\n", iteration);
		id++;
		item = d_sources[id];
		while(item >= 0)
		{
...
//				__threadfence();
			//	printf("push %d with (%d,%d(%d)) in iteration %d\n",item,dist[item],nodesigma[item],d_source_sigma[id],iteration);
			}
			id++;
			item = d_sources[id];
		}
	}
	inwl.level_complete(iteration);
	__threadfence();
	return id;
}
__global__ void drelax2(foru *dist, unsigned hnodes,int *d_sources,unsigned source_count,unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc, unsigned *edgesdst,unsigned *noutgoing,unsigned *gerrno, Worklist inwl, GlobalBarrier gb, unsigned *border, int *d_source_sigma, int *flag, unsigned *child_count, int *child, clock_t *d_o)
{
	unsigned processed_count = 0;
	Worklist *in;
        Worklist *out;
        Worklist *tmp;
	unsigned int iteration = -d_sources[0];
	if (threadIdx.x == 0 && blockIdx.x == 0)
	{
//	printf("[GPU] Before loop\n");
		//printf("[Debug Captain] iteration = %d, dist[845200] = %d, sigma[845200] = %d\n", iteration,dist[845200],nodesigma[845200]);
//		printf("[GPU] Out of loop with iteration = %d, d_sources[0] = %d, flag = %d, inwl.dindex = %d\n",iteration, d_sources[0],*flag, (&inwl)->dindex);
		//processed_count = addSourcesOnCurrentLevel(dist, d_sources, processed_count, inwl, iteration, d_source_sigma, nodesigma, flag); 
		if(d_sources[1] == d_sources[0]) processed_count++;
		processed_count = addSourcesOnCurrentLevel(dist, d_sources, processed_count, inwl, iteration, d_source_sigma, nodesigma, flag);
	}
	...
	in = &inwl; //out = &outwl;
	while(inwl.countVerticesOnLevel(iteration) > 0 )//|| d_sources[processed_count]!=-100000) // && iteration < 30)
        {
		iteration++;
		drelax(dist, hnodes,nodesigma,edgesigma,srcsrc, edgesdst,noutgoing,gerrno, *in, iteration, border, child_count, child);
	...
	//	if (threadIdx.x == 0 && blockIdx.x == 0) printf("[GPU4] iteration = %d\n",iteration);
		...
		//	if(processed_count+1 < source_count)
				processed_count = addSourcesOnCurrentLevel(dist, d_sources, processed_count, *in, iteration, d_source_sigma, nodesigma, flag);
				//inwl.level_complete(iteration);
        	}
		gb.Sync();
		inwl.level_complete(iteration);
		gb.Sync();
        // 		printf("[Debug Captain leaving] iteration = %d, dist[855826] = %d, sigma[855826] = %d\n", iteration,dist[855826],nodesigma[855826]);
//		tmp = in;
  //        	in = out;
    //      	out = tmp;
...
if (threadIdx.x == 0 && blockIdx.x == 0) printf("[GPU] Done MultiS\n");
//if (threadIdx.x == 0 && blockIdx.x == 0) printf("[GPU3] iteration = %d\n",iteration);
}
...
__global__ void drelax2_SS(foru *dist, unsigned hnodes,int *d_sources,unsigned source_count,unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc, unsigned *edgesdst,unsigned *noutgoing,unsigned *gerrno, Worklist inwl, Worklist outwl, int iteration, GlobalBarrier gb, unsigned *border)
{
  ...
  if(iteration == 0)
	...
	//	iteration++;
          ...
          drelax_SS(dist, hnodes,nodesigma,edgesigma,srcsrc, edgesdst,noutgoing,gerrno, *in, *out, iteration,border);
	//iteration++;
...
void lonestar_gpu_MultiS_initialize(unsigned *d_psrc,unsigned hnodes,unsigned hedges)
{
	wl_visited.ensureSpace(2*hedges);      wl2.ensureSpace(2*hedges);
	wl_visited.addMultiSourceSupport(hnodes);
	cudaBindTexture(0, row_offsets, d_psrc, (hnodes + 1) * sizeof(unsigned));
	checkCUDAError("kernel invocation[>1] before MultiS");	
}

void lonestar_gpu_MultiS(unsigned *d_psrc,unsigned *d_noutgoing,unsigned *d_edgessrc,unsigned *d_edgesdst,unsigned hnodes,unsigned hedges,unsigned *dist,unsigned *nodesigma,unsigned *edgesigma,unsigned source_count,int *d_sources,cudaDeviceProp *deviceProp,bool BM_COMP,unsigned *nerr, unsigned *d_border, int *d_sigma, int *flag, cudaStream_t *s_multi, unsigned *child_count, int *child)
{
//	wl1.ensureSpace(2*hedges);      wl2.ensureSpace(2*hedges);
//	printf("[GPU]in lonestar_gpu_MultiS\n");
        Worklist *inwl = &wl_visited, *outwl = &wl2;
//	cudaBindTexture(0, row_offsets, d_psrc, (hnodes + 1) * sizeof(unsigned));
//	GlobalBarrierLifetime gb;
//	printf("[GPU] maximum_residency\n");
...
//	outwl->display_items();
clock_t *d_o = NULL;
//cudaMalloc((void**)&d_o, sizeof(clock_t));
	drelax2 <<<13 * drelax2_max_blocks, BLKSIZE,0, *s_multi>>>(dist, hnodes,d_sources,source_count,nodesigma,edgesigma,d_edgessrc,d_edgesdst,d_noutgoing, nerr, *inwl, gb,d_border,d_sigma, flag, child_count, child, d_o);
//	cudaStreamSynchronize(*s_multi);
//	inwl->display_items();
//	cudaThreadSynchronize();
//	endtime = rtclock();
//	printf("MultiS Time : %f\n", 1000.0*(endtime - starttime));
//	checkCUDAError("kernel invocation[>1] Here");
//	printf("[GPU] Kernal called\n");
//	cudaUnbindTexture(row_offsets);
        return;
}
__global__ void updateBorderChildGPU_kernal(unsigned *borderNodes, int* d_gpu_border_child, int* d_gpu_border_child_count, unsigned *child_count, int* child, unsigned borderCount_gpu)
{
	for(int tid = threadIdx.x + blockIdx.x * blockDim.x; tid < borderCount_gpu; tid += (blockDim.x * gridDim.x))
	{
		int src = borderNodes[tid], num_edges_src;
		num_edges_src = tex1Dfetch(row_offsets, src);
		for(int i=d_gpu_border_child_count[tid]; i<d_gpu_border_child_count[tid+1]; i++)
		{
			child[ num_edges_src + child_count[src] ] = d_gpu_border_child[i];
			child_count[src]++;			
		}
	}
}
void updateBorderChildGPU(unsigned *borderNodes, int* d_gpu_border_child, int* d_gpu_border_child_count, unsigned *child_count, int* child, unsigned borderCount_gpu)
{
	updateBorderChildGPU_kernal <<<13,BLKSIZE>>>(borderNodes, d_gpu_border_child, d_gpu_border_child_count, child_count, child, borderCount_gpu);
}

/*
void backward_gpu(unsigned *d_psrc, unsigned *child_count, int *child, unsigned *nodesigma, float *delta, float *BC, unsigned phase_num)
{
	Worklist *inwl = &wl_visited;
	const size_t drelax2_max_blocks = maximum_residency(drelax2, BLKSIZE, 0);
	gb.Setup(deviceProp->multiProcessorCount * drelax2_max_blocks);
	backward_gpu_kernal <<<13,BLKSIZE>>>();
}

__device__
unsigned processnode_backward()
{
	unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
	unsigned nn;
	typedef cub::BlockScan<int, BLKSIZE> BlockScan;
	...
	while(total_inputs > 0)
	{
		...
		if(inwl.pop_id(id, nn, iteration) )
		{
			if(nn != -1)
			{
				childsize = child_count[nn];
				childoffset = tex1Dfetch(row_offsets, nn);				
			}
		}
		BlockScan(temp_storage).ExclusiveSum(childsize, scratch_offset, total_edges);
		int done = 0;
		int childsdone = 0;
		while(total_edges > 0)
		{
			__syncthreads();
			int i;
			for(i = 0; childsdone + i < childsize && (scratch_offset + i - done) < SCRATCHSIZE; i++)
			{
				gather_offsets[scratch_offset + i - done] = childoffset + childsdone + i;
			}			
			childsdone += i;
			scratch_offset += i;
...
__global__ void backward_gpu_kernal(unsigned *d_psrc, unsigned *child_count, int *child, unsigned *nodesigma, float *delta, float *BC, unsigned phase_num, GlobalBarrier gb, Worklist inwl)
{
	Worklist *in;
	in = &inwl;
	while(phase_num > 0)
	{
		processnode_backward();		
	}	
...
//	cudaMemset(d_check, 0, hnodes*sizeof(unsigned));
	...
	//starttime = rtclock();
	//static Worklist wl1(2*hedges), wl2(2*hedges);
	wl1.ensureSpace(2*hedges);	wl2.ensureSpace(2*hedges);	wl_visited.ensureSpace(hnodes);
...
//		cout<<"Entering Kernal : iteration = "<<iteration<<" sources[0] = "<<sources[0]<<endl;
	...
	//	drelax2 <<<13, BLKSIZE>>> (dist, hnodes,d_sources,source_count,nodesigma,edgesigma,d_edgessrc,d_edgesdst,d_noutgoing, nerr, *inwl, *outwl, iteration, gb);
		...
		cudaThreadSynchronize();
		endtime = rtclock();
		printf("[Deprecated function]MultiS Time : %f\n", 1000.0*(endtime - starttime));
