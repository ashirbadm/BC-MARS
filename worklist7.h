...
unsigned *Worklist::alloc(unsigned allocsize) {
	...
	if (cudaMalloc((void **)&ptr, allocsize * sizeof(unsigned)) != cudaSuccess) {
		...
		printf("%s(%d): Allocating %d failed on GPU.\n", __FILE__, __LINE__, allocsize);
		return NULL;
	}
	return ptr;
}
...
unsigned Worklist::getSize() {
	...
	if (hstart != 0) { printf("\tNOTICE on GPU: hstart = %d.\n", hstart); }
...
__device__ unsigned Worklist::pushRangeEdges(unsigned startindex, unsigned nitems) {
...
//	cudaMemcpy(t,items + hsize ,nitems*sizeof(unsigned),cudaMemcpyDeviceToHost);
